...
def _seed(store, sid):
    for rid, text in [(10, "页表 映射 地址"), (10, "缓存 一致性"), (20, "别的引用 页表")]:
        store.upsert([{"vector": [0.0] * 1024, "text": text, "session_id": sid,
                       "reference_id": rid, "char_start": 0, "char_end": len(text),
                       "source_type": "attachment", "embed_model_id": "fake"}])


def test_retrieve_scoped_to_session_and_references():
    store = FakeVectorStore(); _seed(store, sid=1)
    r = AttachmentRetriever(FakeEmbedder(), store, FakeReranker())
    hits = r.retrieve(session_id=1, reference_ids=[10], query="页表", k=6)
...
def test_empty_when_no_reference_ids():
    store = FakeVectorStore(); _seed(store, sid=1)
    r = AttachmentRetriever(FakeEmbedder(), store, FakeReranker())
    assert r.retrieve(session_id=1, reference_ids=[], query="页表", k=6) == []
