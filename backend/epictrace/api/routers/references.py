...
def _require_session(db: Database, sid: int) -> None:
    with db.session() as s:
        if s.get(AgentSession, sid) is None:
...
@router.get("/cowork/sessions/{sid}/references", response_model=list[ReferenceOut])
def list_references(sid: int, db: Database = Depends(get_db)):
    _require_session(db, sid)
    return ReferenceService(db).list_active(sid)


@router.post("/cowork/sessions/{sid}/references", response_model=ReferenceOut,
             status_code=status.HTTP_201_CREATED)
def add_reference(sid: int, payload: ReferenceCreate, request: Request,
    ...
    try:
        if payload.kind == "external":
            ...
            return svc.add_external(sid, payload.source_path, cw)
        if payload.ingest_record_id is None:
            raise ValueError("ingest_record_id required")
        return svc.add_internal(sid, payload.ingest_record_id, cw)
...
@router.post("/cowork/sessions/{sid}/references/stream")
def add_reference_stream(sid: int, payload: ReferenceCreate, request: Request,
                         db: Database = Depends(get_db)):
    ...
    def _work() -> None:
        ...
        try:
            ref = svc.add_external(sid, payload.source_path, cw,
...
@router.delete("/cowork/sessions/{sid}/references/{rid}", status_code=status.HTTP_204_NO_CONTENT)
def detach_reference(sid: int, rid: int, request: Request, db: Database = Depends(get_db)):
    _require_session(db, sid)
    store = getattr(request.app.state, "attachment_store", None)
    ReferenceService(db, attachment_store=store).detach(sid, rid)
