...
- [ ] **Step 1: 建 venv 与依赖(python3.11 + venv,不用 uv)**
...
include = ["epictrace*"]
```
然后可编辑安装(使 `epictrace` 可被 import + 装好测试依赖):
```bash
.venv/bin/pip install -e ".[dev]"
```
注:`backend/.venv/` 已被根 `.gitignore` 的 `.venv/` 规则排除。所有后续命令用 `.venv/bin/<tool>` 直接调,**不依赖 activate**(shell 状态不跨命令保留)。
...
Run: `cd backend && .venv/bin/pytest tests/test_interfaces.py -v`
...
Run: `cd backend && .venv/bin/pytest tests/test_media_text.py -v`
...
Run: `cd backend && .venv/bin/pytest tests/test_projects_service.py -v`
...
Run: `cd backend && .venv/bin/pytest tests/test_ingest_service.py -v`
...
Run: `cd backend && .venv/bin/pytest tests/test_api_projects.py tests/test_api_files.py -v`
...
- Create: `README.md`
- 依赖:`cd backend && .venv/bin/pip install pywebview`(并把 `pywebview` 加进 `pyproject.toml` 的 `dependencies`)
...
```bash
git add shell/run.py backend/epictrace/api/app.py README.md backend/pyproject.toml
...
1. **后端单测全绿:** `cd backend && .venv/bin/pytest -v` → 全部 PASS。
2. **手测竖切(桌面 app):** `cd frontend && npm run build && cd ../backend && .venv/bin/python ../shell/run.py` → 窗口内:
