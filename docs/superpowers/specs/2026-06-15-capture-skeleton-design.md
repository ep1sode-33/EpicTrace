...
- **录制浮窗**:shell 用 `webview.create_window` 起第二个**无边框 + 置顶 + 可拖动**的小窗口(指向前端 HUD 路由),session 开始时创建、结束时销毁(见 §7)。
...
| `kind` | str | `note` / `clipboard` / `screenshot` / `pause` / `resume` / `audio`(预留)… |
| `ts` | datetime | **绝对时间戳——一等公民**,时间线/回溯/未来引用回跳的根 |
| `payload` | Text | 文本事件存文本;文件事件存相对 staging_dir 的路径;`pause`/`resume` 无 payload |
| `meta` | json | 源相关元数据(截图分辨率、剪贴板来源 app 等) |

**暂停语义**:`pause`/`resume` 作为事件记录(不新增 session 字段)。「实际录制时长」= `started_at→ended_at`(或当前)区间减去所有 `pause→resume` 配对区间。HUD 计时显示该 active elapsed;后端可由事件重建,便于测试。暂停期间 shell 暂停所有原生监听(不采集事件)。
...
录制期间显示一个**常驻置顶的小浮窗**,贴合「你在别的 app 里干活、采集在后台跑」的场景(参考 macOS 录屏 / CleanShot 的录制条)。

- **实现**:shell 用 `webview.create_window` 起第二个窗口,`frameless=True`、`on_top=True`、`easy_drag=True`(可直接拖动)、`resizable=False`、小尺寸;URL 指向前端 HUD 路由(如 `…/#/hud?session={id}`),由同一套 React 渲染。session 开始时 `js_api show_recording_hud(session_id)` 创建,结束时 `hide_recording_hud()` 销毁。
- **按钮(对应图示,从左到右)**:
...
- **降级**:HUD 窗口创建失败(非 macOS / pywebview 限制)→ 记日志降级,采集仍可由主窗采集页的同等控件驱动(笔记/截图/暂停/停止)。开发态(浏览器)无第二窗口 → 主窗内联一个 HUD 条占位。

## 8. 数据流 / 生命周期

```
开 session(选源,后端建 capture_session + staging_dir;shell 起原生监听 + 显示 HUD)
  → 事件实时累积(后端 SSE / 轮询给前端 live feed;HUD 显示计时)
  → 暂停/继续(pause/resume 事件,monitors 暂停/恢复)
  → 停 session(status=staged,shell 停所有原生监听 + 销毁 HUD)
...
`Organizer` 默认实现 `PassthroughOrganizer`:不调 LLM,产出「整段 session 归到用户手选的一个 Project」的 `OrganizationProposal`。前端在暂存区给一个「指派到 Project」入口(选已有 Project)。提交即执行 §8 的物化 + 入库 + 索引。

真·归类 Agent(进哪个 Project / 是否新建 / 子文件夹 / 派生文件 / 复核)是后续 plan:届时新增一个 `Organizer` 实现替换默认,**§8 的 call site 与物化/入库逻辑不变**。

## 10. API(`/api/capture/...`)与 `js_api`

- `POST /api/capture/sessions {sources}` → 建并开始 session(已有活动 session 则 409)。
- `POST /api/capture/sessions/{id}/pause` / `/resume` → 记 `pause`/`resume` 事件。
- `POST /api/capture/sessions/{id}/stop` → 结束(status=staged)。
- `GET /api/capture/sessions` / `GET /api/capture/sessions/{id}` → 列表 / 详情(含事件)。
- `POST /api/capture/sessions/{id}/events` → 追加事件(笔记来自前端/HUD;截图/剪贴板来自 shell)。
...
- shell `js_api` 新增:`capture_screenshot(region?) -> 路径`、`start_capture_monitors(session_id, sources)`、`stop_capture_monitors()`(注册/注销全局热键 + 剪贴板轮询 + 暂停态)、`show_recording_hud(session_id)`、`hide_recording_hud()`。

## 11. 权限与降级(macOS)

- 截图需**屏幕录制**权限、全局热键需**辅助功能**权限:首次触发引导授权;被拒 → 该源标灰 + 提示,不崩、不静默丢。
- 剪贴板轮询**仅 session 活动期间**,暂停即停、停止即止;只读不写。
- HUD 窗口创建失败 → 降级,采集由主窗控件驱动(§7)。
- **opt-in**:不开 session 不采集;源默认**全关**,显式开。
- 开发态(浏览器无 `window.pywebview`):笔记可用;剪贴板/截图原生源标灰提示「需在桌面 app 内」;HUD 内联占位。
...
- 暂停态下到达的事件(理论上不应有,因 monitors 已停)→ 后端按当前状态照常记录,不特殊处理。
- organize 时 Project 不存在 / 已 organized → 4xx。
- 删 session → 连带删 staging 文件(已 organized 的、已复制进 Project 的文件**不删**——那是用户的库内文件)。

## 13. 测试策略

- **后端 TDD(全 fake)**:`FakeClock` 注入时间戳;假 shell-capture(直接 POST 构造事件);假 `Organizer`/复用现有 fake embedder+store。覆盖:session 生命周期(开/暂停/继续/停/单一活动 409)、event 追加与 `ts` 排序、**active elapsed 由 pause/resume 事件重建**、staging 落盘与删除、`PassthroughOrganizer` 物化 → `ingest_file(ingest_method="session", source_session_id=...)` → 索引(断言文本事件入索引、截图文件落库 `extracted_text=""`)、改名、organize 幂等/409。
- **shell 原生四件**(截图 / 全局热键 / 剪贴板监听 / HUD 窗口):薄封装 + **手测**(无自动化,同 `read_clipboard_files` 的既有处理);异常一律降级为空/日志。
- **前端**:`npm run build`(含 HUD 路由)。
